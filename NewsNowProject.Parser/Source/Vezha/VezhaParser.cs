...
{
    public class VezhaParser : IParser<List<Article>>
    {
        public IParserSettings parserSettings { get; set; }
        public VezhaParser()
        {
            parserSettings = new VezhaSettings();
        }
        public List<Article> Parse(IElement element)
        {
            var list = new List<Article>();
            var items = element.QuerySelectorAll("a")
                               .Where(item => item.HasAttribute("data-wpel-link") && item.GetAttribute("data-wpel-link").Contains("internal"));
