...
{
    public class VinnitsaOkParser : IParser<List<Article>>
    {
        public IParserSettings parserSettings { get; set; }
        public VinnitsaOkParser()
        {
            parserSettings = new VinnitsaOkSettings();
        }
        public List<Article> Parse(IElement element)
        {
            var list = new List<Article>();
            var items = element.GetElementsByClassName(".ng-isolate-scope li a");

            foreach (var item in items.Reverse())
            {
                var url = HelperParser.GetUrl(parserSettings.BaseUrl, item.Attributes.GetNamedItem("href").Value);
                var title = item.GetElementsByTagName("span").FirstOrDefault()?.TextContent.Trim();
        ...
        {
            return document.GetElementById("h-news");
        }
