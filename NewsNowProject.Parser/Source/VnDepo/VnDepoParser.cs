...
{
    public class VnDepoParser : IParser<List<Article>>
    {
        public IParserSettings parserSettings { get; set; }
        public VnDepoParser()
        {
            parserSettings = new VnDepoSettings();
        }
        public List<Article> Parse(IElement element)
        {
            var list = new List<Article>();
            var items = element.GetElementsByClassName("link_title_news");

            foreach (var item in items.Reverse())
            {
                var url = HelperParser.GetUrl(parserSettings.BaseUrl, item.Attributes.GetNamedItem("href").Value);
                var title = item.GetElementsByClassName("title_news").FirstOrDefault()?.TextContent.Trim();
        ...
        {
            return document.GetElementsByClassName("list_news").FirstOrDefault();
        }
