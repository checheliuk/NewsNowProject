...
{
    public class VinGovUaParser : IParser<List<Article>>
    {
        public IParserSettings parserSettings { get; set; }
        public VinGovUaParser()
        {
            parserSettings = new VinGovUaSettings();
        }
        public List<Article> Parse(IElement element)
        {
            var list = new List<Article>();
            var items = element.QuerySelectorAll(".right-article a");
        ...
        {
            return document.GetElementById("sp-component").GetElementsByClassName("blog-featured").FirstOrDefault();
        }
