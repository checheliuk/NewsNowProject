...
{
    public class MistoVnUaParser : IParser<List<Article>>
    {
        public IParserSettings parserSettings { get; set; }
        public MistoVnUaParser()
        {
            parserSettings = new MistoVnUaSettings();
        }
        public List<Article> Parse(IElement element)
        {
            var list = new List<Article>();
            var items = element.QuerySelectorAll(".dl-horizontal a");
        ...
        {
            return document.GetElementsByClassName("posts").FirstOrDefault();
        }
